...
### Notes Formatting ###
assetship= """{shippingUsed} --- GCA-{asset} --- {STATUS}
Model - {model}
...
def worldShipData(x,y,listingStatus):
    ...
    assetshipprint = assetship.format(model = y,shippingUsed = shippingUsed,asset=x,STATUS = listingStatus,trackingNumber=trackingNumber,shipToInsert=shipToAttention,shipDate=shipDate.date(),address=address,address2=address2,zip=zipCode, city=city,shipmentStatus='Delivered').strip()
...
def upsData(x,y,listingStatus):
    ...
    if  shipToInsert != "SCA":
        ...
        print("\n")
        shippingUsed = "UPS"
        assetshipprint = assetship.format(model = y,shippingUsed=shippingUsed, asset=x,STATUS = listingStatus,trackingNumber=trackingNumber,shipToInsert=shipToInsert,shipDate=shipDate.date(),address=address,address2=address2,zip="", city=city,shipmentStatus=shipmentStatus).strip()
        print(assetshipprint)
        list_c.append(assetshipprint)
        list_e.append(assetshipprint)
        return assetshipprint
